-- 064: Freeze receipt toggles + labels on each transaction (same moment as cash box / sender snapshots).
-- Reprints use transaction columns; NULL on legacy rows = fall back to live cash box in the client.

BEGIN;

ALTER TABLE public.transactions
  ADD COLUMN IF NOT EXISTS receipt_show_logo boolean,
  ADD COLUMN IF NOT EXISTS receipt_show_addresses boolean,
  ADD COLUMN IF NOT EXISTS receipt_show_tracking boolean,
  ADD COLUMN IF NOT EXISTS receipt_show_additional boolean,
  ADD COLUMN IF NOT EXISTS receipt_show_note boolean,
  ADD COLUMN IF NOT EXISTS receipt_show_signatures boolean,
