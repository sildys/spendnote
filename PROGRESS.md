...
## Where we are now (last updated: 2026-05-28 16:25 — V1 #7 5/5 KÉSZ + V1 #4 ✅ KÉSZ: BUG #2 trial_ends_at Stripe webhook sync (stripe-webhook handler payload-bővítés sub.trial_start + sub.trial_end ISO-string-re) + BUG #1 CB access grant invite (B2 PRE-GRANT INVITE-FLOW komplett: migration 067 invites.cash_box_ids JSONB + spendnote_create_invite p_cash_box_ids param + spendnote_accept_invite_v2 honor pre-granted CB-ids fallback first-CB-re + supabase-config.js invite() új paraméter + team-page.js Invite modal CB checkbox-picker + Cash Box Access modal pending invite pre-grant display). 0 lints. Korábbi BATCH-2 SEO ÉLESÍTÉS (3 oldal: D6+D7+D8) szintén MA délután, commit 5eff726. **FELHASZNÁLÓI MA OPCIONÁLIS TEENDŐ:** (1) GSC manual reindex-request a 3 új BATCH-2 URL-re; (2) Supabase SQL Editor: migration 067 manual futtatása; (3) Supabase Edge Functions: stripe-webhook manual redeploy. Következő SEO: 2026-05-31 vas BATCH-3. Következő V1: #8 GA4 5 event vagy #6 CAN-SPAM guards.):
