    ...
    try {
        ...
        if (!hasSession) {
            try {
                ...
                hasSession = !error && Boolean(data?.session?.access_token);
            } catch (_) {
                hasSession = false;
            }
        }
        ...
        const invokeOnce = async () => {
            try {
                const { data, error } = await supabaseClient.functions.invoke('send-user-event-email', {
                    body: payload || {}
                });
                if (error) {
                    return { success: false, error: String(error?.message || error), data: null };
                }
                return { success: true, error: '', data: data || null };
            } catch (e) {
                return { success: false, error: String(e?.message || e || 'Email event send failed.'), data: null };
            }
        };
        ...
        const firstMsg = String(first.error || '').toLowerCase();
        if (firstMsg.includes('invalid jwt') || firstMsg.includes('unauthorized') || firstMsg.includes('jwt')) {
            try {
            ...
            if (second.success) return { success: true, data: second.data };
            __spendnoteLogBackendError('userEventEmail.send', {
                status: 401,
                message: second.error || first.error,
                requestId: '',
                payload: second.data || null
            }, { payload });
            return { success: false, error: `Email event send failed: ${second.error || first.error}` };
        }

        __spendnoteLogBackendError('userEventEmail.send', {
            status: 0,
            message: first.error || 'Email event send failed.',
            requestId: '',
            payload: first.data || null
        }, { payload });
        return { success: false, error: `Email event send failed: ${first.error || 'Unknown error'}` };
    } catch (e) {
...
window.SpendNoteFeatures = {
    ...
    _FLAGS: {
        preview:  { can_create_transaction: true,  can_export_csv: true,  can_export_pdf: true,  can_send_email_receipt: true,  can_upload_logo: true,  can_customize_labels: true,  can_customize_id_prefix: true,  can_invite_members: true,  can_add_cash_box: true,   max_cash_boxes: Infinity, max_users: Infinity, max_transactions: 200,      trial_days: Infinity },
        free:     { can_create_transaction: true,  can_export_csv: true,  can_export_pdf: true,  can_send_email_receipt: true,  can_upload_logo: true,  can_customize_labels: true,  can_customize_id_prefix: true,  can_invite_members: true,  can_add_cash_box: true,   max_cash_boxes: Infinity, max_users: 3,        max_transactions: Infinity, trial_days: 14 },
        standard: { can_create_transaction: true,  can_export_csv: true,  can_export_pdf: true,  can_send_email_receipt: false, can_upload_logo: true,  can_customize_labels: false, can_customize_id_prefix: false, can_invite_members: false, can_add_cash_box: true,   max_cash_boxes: 2,  max_users: 1,        max_transactions: Infinity, trial_days: Infinity },
        pro:      { can_create_transaction: true,  can_export_csv: true,  can_export_pdf: true,  can_send_email_receipt: true,  can_upload_logo: true,  can_customize_labels: true,  can_customize_id_prefix: true,  can_invite_members: true,  can_add_cash_box: true,   max_cash_boxes: Infinity, max_users: 3,        max_transactions: Infinity, trial_days: Infinity }
    },
