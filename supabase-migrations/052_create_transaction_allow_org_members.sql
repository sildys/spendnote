-- 052: Allow invited team members to record transactions on assigned cash boxes.
-- Previously spendnote_create_transaction required cash_boxes.user_id = auth.uid(), which
-- blocked User-role members (box row is owned by team owner).
...
BEGIN
  ...
  -- Box owner (solo / row owner), or org owner/admin, or User with cash_box_memberships
